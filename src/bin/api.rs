...
async fn read_csv_header() -> Result<impl warp::Reply, Infallible> {
    ...
    let df =
        CsvReader::from_path("/home/amine/Documents/internship/projects/wb_api/src/titanic.csv")
    ...
    let header: Vec<String> = df
        .get_columns() //get the columns of the DataFrame
...
async fn read_csv_row(row_index: usize) -> Result<impl warp::Reply, Infallible> {
    ...
    if row_index < df.height() {
        ...
        let row_data = df
            ...
            .0
            .iter() //iterate over the values in the Row
